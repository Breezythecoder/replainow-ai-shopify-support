...
const Pricing = () => {
  return <section id="preise" aria-labelledby="pricing-heading" className="py-16 md:py-24">
      <div className="container">
        <h2 id="pricing-heading" className="font-display text-3xl md:text-4xl font-semibold text-center mb-4">Mehr Support-Power für weniger Geld als ein einziger Mitarbeiter.</h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-6 max-w-6xl mx-auto">
            {plans.map(p => <article key={p.name} className={`rounded-2xl border p-6 bg-card flex flex-col ${p.popular ? 'shadow-brand' : ''}`} style={p.popular ? {
          borderColor: 'hsl(var(--brand-popular))',
          backgroundColor: 'hsl(var(--brand-popular) / 0.06)'
        } : undefined}>
                <header className="mb-4">
                  {'badge' in p && p.badge ? <span className="mb-2 inline-flex items-center rounded-full px-2.5 py-1 text-xs font-medium" style={{
              backgroundColor: 'hsl(var(--brand-popular) / 0.18)',
              border: '1px solid hsl(var(--brand-popular))'
            }}>
                      {p.badge}
                    </span> : null}
