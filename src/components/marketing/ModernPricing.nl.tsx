            ...
            const Card = (
              <div className="glass-card rounded-3xl p-6 sm:p-8 h-full flex flex-col interactive-card border border-white/40 shadow-brand hover-lift">
                ...
                <div className="space-y-3">
                  <Button asChild variant={plan.highlight ? 'cta' : 'glass'} size="lg" className="w-full text-sm sm:text-base"><a href={`${OAUTH_URL}?utm_source=site&utm_campaign=${encodeURIComponent(plan.name)}&utm_content=pricing`}>Gratis Proberen</a></Button>
                  <div className="flex items-center justify-center gap-2 text-xs text-slate-500"><img src="/images/shopify-logo-black.svg" alt="Shopify Logo" className="h-3 sm:h-4 w-auto opacity-80" loading="lazy" /><span>In Shopify App Store</span></div>
                </div>
              </div>
            );
...
export default ModernPricingNl;