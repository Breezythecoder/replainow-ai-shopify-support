...
const AIKnowledgeBanner = () => {
  ...
  // Data sources from i18n with SAFE fallback
  let leftSources = defaultLeftSources;
  let rightSources = defaultRightSources;
  let liveEvents = defaultLiveEvents;
  
  try {
    const leftData = t('marketing.aiKnowledge.dataSourcesLeft', { returnObjects: true });
    if (Array.isArray(leftData) && leftData.length > 0) {
      leftSources = leftData;
    }
    
    const rightData = t('marketing.aiKnowledge.dataSourcesRight', { returnObjects: true });
    if (Array.isArray(rightData) && rightData.length > 0) {
      rightSources = rightData;
    }
    
    const eventsData = t('marketing.aiKnowledge.liveEvents', { returnObjects: true });
    if (Array.isArray(eventsData) && eventsData.length > 0) {
      liveEvents = eventsData;
    }
  } catch (e) {
    console.warn('AIKnowledgeBanner: Using fallback data', e);
  }
