...
async function completeAudit() {
  ...
  try {
    ...
    const chrome = await require('playwright').chromium.launch();
    const options = {
      logLevel: 'info',
      output: 'json',
    ...
    const runnerResult = await lighthouse('https://replainow.com', options);
    fs.writeFileSync(`${auditDir}/15-lighthouse-audit.json`, JSON.stringify(runnerResult.lhr, null, 2));
