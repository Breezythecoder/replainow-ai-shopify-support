      ...
      headings.forEach((heading, index) => {
        ...
        if (level > lastLevel + 1) {
          issues.push({
            ...
            selector: heading.id ? `#${heading.id}` : heading.className ? `.${heading.className.split(' ')[0]}` : heading.tagName.toLowerCase()
          });
        }
        lastLevel = level;
      });
  ...
  async runUXAudit() {
    ...
    const designConsistency = await this.page.evaluate(() => {
      ...
      return {
        ...
        consistentButtons: consistentButtons
      };
    });
    ...
    const mobileCheck = await this.page.evaluate(() => {
      ...
      const touchFriendlyRatio = clickableElements.length > 0 ? touchFriendly / clickableElements.length : 1;
      
      return {
        hasViewport: hasViewport,
        viewportContent: viewport?.content || '',
        touchFriendly: touchFriendlyRatio
      };
    });
...
// Helper functions removed - now inline in page evaluations
